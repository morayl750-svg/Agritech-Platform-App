...
import { MapPin, Phone, Mail, Award, Edit3, Sprout, ShoppingBag, MessageSquare, User as UserIcon } from 'lucide-react'
...
export default function Profile() {
  ...
  const [name, setName] = useState(user?.full_name || 'AgriSmart User')
  const [email, setEmail] = useState(user?.email || 'user@agrismart.so')
  const [phone, setPhone] = useState(user?.phone || '+252 61 555 0192')
  const [city, setCity] = useState(user?.city || 'Muqdisho')
  const [district, setDistrict] = useState(user?.district || 'Hodan')
  const [region, setRegion] = useState(user?.region || 'Lower Shabelle (Afgooye Sector)')
  const [farmSize, setFarmSize] = useState(user?.farm_size || '15 Hectares (Sorghum & Sesame)')
    ...
    <div className="w-full flex flex-col gap-6 items-stretch pb-10 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              >
                <Award size={10} /> {user?.role === 'Admin' ? 'Maamulaha Guud (Admin)' : 'Macmiil / Beeraley (User)'}
              </span>
            </div>
            {user?.username && (
              <p className="text-xs font-mono text-emerald-700 font-medium">@{user.username}</p>
            )}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-2xs space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div>
            <span className="text-gray-400 block font-medium mb-1">Username</span>
            <p className="text-gray-900 font-medium flex items-center gap-1.5 font-mono">
              <UserIcon size={13} className="text-gray-400" /> @{user?.username || 'user'}
            </p>
          </div>

          <div>
            <span className="text-gray-400 block font-medium mb-1">Email / Gmail Address</span>
          ...
          <div>
            <span className="text-gray-400 block font-medium mb-1">Magaalada & Xaafadda (Location)</span>
            <p className="text-gray-900 font-medium flex items-center gap-1.5">
              <MapPin size={13} className="text-gray-400" /> {city || 'Muqdisho'}{district ? ` - ${district}` : ''}
            </p>
          </div>
