...
export function useDashboardData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Execute concurrent dynamic operational queries
      const [
      ...
      if (weatherData) {
        setWeatherInfo({
          ...
          tempCelsius: Number(weatherData.temperature),
          condition: weatherData.condition,
          metrics: [
            { label: 'Humidity', value: weatherData.humidity },
            { label: 'Wind', value: weatherData.wind_speed },
            { label: 'Rain', value: weatherData.rain_mm },
          ],
